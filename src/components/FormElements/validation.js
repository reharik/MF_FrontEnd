...
let validators = require('./validatorEnum.js');
...
let validationRules = (function () {
  return {
    ...
    rangelength(value, param1, param2) {
...
let validationRunner = (function () {
  return function validate(rules, value, param1, param2) {
    let _valid = {valid: true};
    if (!rules || !Array.isArray(rules) || rules.length == 0 || ((!value || value === '') && !_.any(rules, function (item) {
        return item === validators.REQUIRED;
      }))) {
      return _valid;
    }
    let errors = _.map(rules, function (rule) {
